...
IGridState > {
    ...
    flatHeader = () => {
        let allColumns = this.props.headers.filter((x) => x.name === this.state.visibleHeader).map((header : IHeader) => {
    ...
    render() {
        let allSelectionFilters = this.flatHeader().filter((column) => column.type === "select");
