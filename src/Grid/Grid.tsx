...
export const GridContext = createContext<IGridContext & ISortable>({
  ...
  bottom: 0,
  data: {
    get: (sort: ISortStats, filters: IFilter[]) => new Promise(() => {}),
    getTotal: (sort: ISortStats, filters: IFilter[]) => new Promise(() => {}),
  },
  filters: [],
  headersContext: [],
  items: [],
...
export default function Grid(props: IGridProps) {
  ...
  useEffect(() => {
    const ReloadData = async () => {
      ...
      let loadingElements = await loadPage.getPage(ScrollDirection.Initial, {
      ...
      let totalValue = await props.data.getTotal(sort, filters);

      updateAllPages(totalValue);
