...
export class ScrollPage {
  ...
  async scrollUp(
    ...
    if (event.target.scrollTop === 0) {
      if (top >= 0) {
        ...
        let newCache = await this.loadPage.getPage(ScrollDirection.Up, {
          pageSize: pageSize,
          top: top,
          bottom: bottom,
          sort: sort,
  ...
  async scrollDown(
    ...
    const loadOnScroolDown = async (event: any) => {
      ...
      if (isBottomReached(event)) {
        if (loadedPages + offset < allPages) {
          ...
          let newCache = await this.loadPage.getPage(ScrollDirection.Down, {
            pageSize: pageSize,
            top: top,
            bottom: bottom,
            sort: sort,
          ...
          if (newCache.length < pageSize) {
            let offsetCache = await this.loadPage.getPage(
